  ...
  ({ className, type, label, error, hint, id, ...props }, ref) => {
    ...
    return (
      <div className="space-y-1.5">
        ...
        />
        {error && (
          <p id={`${inputId}-error`} className="text-sm text-red-600 dark:text-red-400" role="alert">{error}</p>
        )}
        {hint && !error && (
          <p id={`${inputId}-hint`} className="text-sm text-gray-500 dark:text-gray-400">{hint}</p>
        )}
      </div>
    )
  }
