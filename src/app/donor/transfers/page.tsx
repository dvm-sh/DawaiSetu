...
export default function DonorTransfersPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-1">Track all your medicine transfers</p>
      </div>
      ...
      {isLoading ? <TableSkeleton /> : transfers.length === 0 ? (
        <EmptyState title="No transfers yet" description="Your transfers will appear here once recipients request your medicines" />
      ) : (
        <Card padding={false}>
                ...
                {transfers.map(t => (
                  <tr key={t.id as string} className="border-b border-gray-100 hover:bg-gray-50/50">
                    ...
                    <td className="py-3 px-4 hidden md:table-cell">
                      <p className="font-medium text-gray-900">{(t.recipientOrg as Record<string, string>)?.name}</p>
                      <p className="text-xs text-gray-500">{(t.recipientOrg as Record<string, string>)?.city}</p>
                    </td>
                    ...
                    <td className="py-3 px-4">
                      <Link href={`/donor/transfers/${t.id}`}>
