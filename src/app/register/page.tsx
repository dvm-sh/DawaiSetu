    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-gray-50">
        <div className="max-w-md w-full bg-white rounded-2xl p-8 text-center shadow-sm border border-gray-200">
          ...
          <p className="text-gray-600 mb-6">Your organization has been registered and is pending verification by our admin team. You will be notified once approved.</p>
      ...
      <div className="flex-1 flex items-center justify-center p-4 sm:p-6 pb-12">
        <div className="w-full max-w-lg">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 sm:p-8">
            <h1 className="text-2xl font-bold text-gray-900 mb-2">Register Your Organization</h1>
            <p className="text-gray-600 mb-6">Join DawaiSetu to donate or receive medicines</p>

            {/* Progress */}
            <div className="flex items-center gap-2 mb-8">
              {[1, 2, 3].map(s => (
                <div key={s} className="flex-1 flex items-center gap-2">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${step >= s ? 'bg-teal-600 text-white' : 'bg-gray-100 text-gray-400'}`}>{s}</div>
                  {s < 3 && <div className={`flex-1 h-0.5 ${step > s ? 'bg-teal-600' : 'bg-gray-200'}`} />}
                </div>
              ))}
            </div>
            ...
            {step === 1 && (
              <div className="space-y-4">
                ...
                <button
                  ...
                  className={`w-full p-5 rounded-xl border-2 text-left transition-all hover:border-teal-500 ${role === 'DONOR' ? 'border-teal-500 bg-teal-50' : 'border-gray-200'}`}
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-semibold text-gray-900">Donate Medicines</p>
                      <p className="text-sm text-gray-500">I have surplus medicines to share</p>
                    </div>
                  </div>
                </button>
                <button
                  onClick={() => { setRole('RECIPIENT'); setStep(2) }}
                  className={`w-full p-5 rounded-xl border-2 text-left transition-all hover:border-teal-500 ${role === 'RECIPIENT' ? 'border-teal-500 bg-teal-50' : 'border-gray-200'}`}
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-semibold text-gray-900">Receive Medicines</p>
                      <p className="text-sm text-gray-500">I need medicines for my patients</p>
                    </div>
                  </div>
                </button>
              </div>
